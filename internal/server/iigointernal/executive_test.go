...
func TestAllocationRequests(t *testing.T) {
	cases := []struct {
	...
	}{
		{
			...
			input: allocInput{resourceRequests: map[shared.ClientID]shared.Resources{
				...
				shared.Teams["Team3"]: 15,
				shared.Teams["Team4"]: 20,
				shared.Teams["Team5"]: 25,
				shared.Teams["Team6"]: 30,
			}, commonPoolResource: 100},
			reply: map[shared.ClientID]shared.Resources{
				shared.Teams["Team1"]: calcExpectedVal(5, 105, 100),
				shared.Teams["Team2"]: calcExpectedVal(10, 105, 100),
				shared.Teams["Team3"]: calcExpectedVal(15, 105, 100),
				shared.Teams["Team4"]: calcExpectedVal(20, 105, 100),
				shared.Teams["Team5"]: calcExpectedVal(25, 105, 100),
				shared.Teams["Team6"]: calcExpectedVal(30, 105, 100),
			},
			want: true,
		},
		{
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			...
			executive := &executive{
				PresidentID:     shared.Teams["Team1"],
...
func TestBroadcastTaxation(t *testing.T) {
	...
	cases := []struct {
	...
	}{
		{
			name: "Simple test",
			bPresident: executive{
				PresidentID:     shared.Teams["Team4"],
				clientPresident: &baseclient.BasePresident{},
				logger:          logging,
				gameState: &gamestate.GameState{
			...
			clientReports: map[shared.ClientID]shared.ResourcesReport{
				shared.Teams["Team1"]: {ReportedAmount: 30, Reported: true},
				shared.Teams["Team2"]: {ReportedAmount: 9, Reported: true},
				shared.Teams["Team3"]: {ReportedAmount: 15, Reported: true},
				shared.Teams["Team4"]: {ReportedAmount: 20, Reported: true},
				shared.Teams["Team5"]: {ReportedAmount: 25, Reported: true},
				shared.Teams["Team6"]: {ReportedAmount: 40, Reported: true},
			},
			commonPool: 150,
		},
		{
			...
			clientReports: map[shared.ClientID]shared.ResourcesReport{
				shared.Teams["Team1"]: {ReportedAmount: 30, Reported: true},
				shared.Teams["Team2"]: {Reported: false},
				shared.Teams["Team3"]: {ReportedAmount: 15, Reported: true},
				shared.Teams["Team4"]: {Reported: false},
				shared.Teams["Team5"]: {ReportedAmount: 25, Reported: true},
				shared.Teams["Team6"]: {Reported: false},
			},
			commonPool: 150,
		},
	}
